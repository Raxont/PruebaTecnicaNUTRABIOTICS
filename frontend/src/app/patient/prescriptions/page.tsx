  ...
  const loadPrescriptions = useCallback(
    async (f: Filters, page: number) => {
      ...
      try {
        ...
        // Client-side filter by doctor name
        let data = result.data;
        if (f.doctorSearch.trim()) {
          const q = f.doctorSearch.trim().toLowerCase();
          data = data.filter((p) => {
            const fullName =
              `${p.doctor?.user.firstName ?? ''} ${p.doctor?.user.lastName ?? ''}`.toLowerCase();
            const email = (p.doctor?.user.email ?? '').toLowerCase();
      ...
      <main className="main-shell">
        ...
        <div className="card" style={{ marginBottom: 18 }}>
          <div className="filter-grid">
            ...
            {/* Doctor search */}
            <div className="filter-field">
              <label className="filter-label">Médico</label>
              <input
                className="input"
                type="text"
                placeholder="Nombre o email del médico"
                value={filters.doctorSearch}
                onChange={(e) => setFilters((f) => ({ ...f, doctorSearch: e.target.value }))}
          ...
          {loading ? (
            ...
            <div className="empty-state">
              {hasActiveFilters
                ...
                : 'No tienes prescripciones disponibles.'}
            </div>
          ) : (
            <>
